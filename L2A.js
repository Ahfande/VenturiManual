...
// Button L2A
// OFF
const BtnL2AOFF = document.getElementById("btnL2AOFF").addEventListener('click', () =>{
    database.ref("Manual/L2A/")
    .set({ status: "OFF" })
    .then(() => {
    console.log("L2A OFF");
...
const BtnL2AON = document.getElementById("btnL2AON").addEventListener('click', () =>{
    database.ref("Manual/L2A/")
    .set({ status: "ON" })
    .then(() => {
    console.log("L2A ON");
...
window.onload = (() => {
    database.ref("Manual/L2A/").get().then((snapshot) => {
